...
from locators.workshop_setup_locators import WorkshopLocators
...
class WorkshopPage(BasePage):
    ...
    def is_workshop_exists(self, workshop_data: dict):
        ...
        try:
            ...
            # 一次性获取所有行
            rows = self.find_elements(WorkshopLocators.TABLE_ROWS, allow_empty=True)
            if not rows:
                return False
            # 预先处理需要检查的字段和列索引
            check_fields = []
            for field in workshop_data:
            ...
            for row in rows:
                ...
                for field, col_index in check_fields:
                    expected_value = workshop_data[field]
